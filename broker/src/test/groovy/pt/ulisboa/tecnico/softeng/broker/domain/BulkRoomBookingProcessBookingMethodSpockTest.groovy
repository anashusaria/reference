...
class BulkRoomBookingProcessBookingMethodSpockTest extends SpockRollbackTestAbstractClass {
    ...
    def 'success'() {
        ...
        hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, BROKER_NIF, BROKER_IBAN,
    ...
    def 'success twice'() {
        ...
        0 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, BROKER_NIF, BROKER_IBAN,
                bulk.getId())
        bulk.getReferences().size() == 2
    ...
    def 'one hotel exception'() {
        ...
        1 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE, BROKER_NIF, BROKER_IBAN,
    ...
    def 'max minus one hotel exception'() {
        ...
        (BulkRoomBooking.MAX_HOTEL_EXCEPTIONS - 1) * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE,
                BROKER_NIF, BROKER_IBAN, bulk.getId()) >> { throw new HotelException() }
    ...
    def 'hotel exception value is reset by remote exception'() {
        ...
        1 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE,
                BROKER_NIF, BROKER_IBAN, bulk.getId()) >> { throw new RemoteAccessException() }
    ...
    def 'one remote exception'() {
        ...
        and: 'the second invocation returns data'
        1 * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE,
                BROKER_NIF, BROKER_IBAN, bulk.getId()) >> { new HashSet<>(Arrays.asList("ref1", "ref2")) }
        and: 'the bulk booking is not cancelled'
        !bulk.getCancelled()
        and: 'the references are stored'
        bulk.getReferences().size() == 2
    ...
    def 'max remote exception'() {
        ...
        hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE,
                BROKER_NIF, BROKER_IBAN, bulk.getId()) >> { throw new RemoteAccessException() }
    ...
    def 'remote exception value is reset by hotel exception'() {
        ...
        (BulkRoomBooking.MAX_REMOTE_ERRORS - 1) * hotelInterface.bulkBooking(NUMBER_OF_BULK, ARRIVAL, DEPARTURE,
                BROKER_NIF, BROKER_IBAN, bulk.getId()) >> { throw new RemoteAccessException() }
